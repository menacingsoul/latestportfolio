...
export async function PATCH(request: Request) {
  try {
    const { tagline, bio, image, email, resume, github, linkedin } =
      await request.json();
    ...
    return NextResponse.json(
      { error: "Error updating Adarsh details" },
      { status: 500 }
    );
  }
}
